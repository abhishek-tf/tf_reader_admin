...
/** The catalogue-wide Institutions page: every institution, filterable by status. SUPER_ADMIN
 * only — an INSTITUTION_ADMIN is scoped to one institution, which has nothing to filter or
 * browse, so InstitutionsScreen sends that role to MyInstitutionScreen instead of here. Create
 * and edit are pages, rendered through this screen's own `<Outlet/>` as a modal overlaying this
 * list — see App.jsx's nested `/institutions` routes and the same pattern already used by
 * Publishers/Books. Clicking an institution's name goes to its own detail page
 * (`/institutions/:id`), where its entitlements and the books under them live — not an inline
 * card at the bottom of this list. */
export default function InstitutionCatalogueScreen() {
